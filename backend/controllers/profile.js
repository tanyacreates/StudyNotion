...
// ================ instructor Insights (detailed metrics) ================
exports.instructorInsights = async (req, res) => {
    try {
        ...
        const courseInsights = courses.map((course) => {
            ...
            const revenue = enrolledCount * (course.price || 0);
            const reviewsCount = course.ratingAndReviews?.length || 0;

            let courseRatingSum = 0;
            (course.ratingAndReviews || []).forEach((r) => {
                courseRatingSum += parseFloat(r.rating) || 0;
            });
            const avgRating = reviewsCount > 0 ? courseRatingSum / reviewsCount : 0;

            totalStudents += enrolledCount;
            totalRevenue += revenue;
            totalReviews += reviewsCount;
            ratingSum += courseRatingSum;

            if (course.status === 'Published') publishedCount += 1;
            else draftCount += 1;

            const categoryName = course.category?.name || 'Uncategorized';
            categoryDistribution[categoryName] = (categoryDistribution[categoryName] || 0) + enrolledCount;
            ...
            return {
                ...
                // approximate "views" as enrolledCount * 8 (proxy metric since real view tracking is not in DB)
                views: enrolledCount * 8 + reviewsCount * 3,
                clicks: enrolledCount * 3 + reviewsCount,
        ...
        const topCoursesByRevenue = [...courseInsights]
            .sort((a, b) => b.revenue - a.revenue)
            .slice(0, 5);

        // top 5 courses by enrollments
        const topCoursesByEnrollments = [...courseInsights]
        ...
        return res.status(200).json({
            ...
            message: 'Instructor insights fetched successfully'
        });
    } catch (error) {
        console.error('Error while fetching instructor insights', error);
        return res.status(500).json({
            success: false,
            error: error.message,
            message: 'Error while fetching instructor insights'
        });
    }
}